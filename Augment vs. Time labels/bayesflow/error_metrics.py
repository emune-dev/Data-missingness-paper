...
def bar_chart(ax, means, column, category, se=None, y_lim=None, y_ticks=None):
    ...
    if se is not None:
        ax[column].bar(x=[1,2], height=means, yerr=se, capsize=10, width=1.0, color=['orange', 'dodgerblue'])
    else: 
        ax[column].bar(x=[1], height=means[0], width=1.0, color=['orange'], label='Augment by $0/1$')
        ax[column].bar(x=[2], height=means[1], width=1.0, color=['dodgerblue'], label='Time labels')
