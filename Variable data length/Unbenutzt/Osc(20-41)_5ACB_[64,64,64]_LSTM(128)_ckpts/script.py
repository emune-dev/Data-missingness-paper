...
n_max = 41
n_min = 20
time_points = np.linspace(0, 10, n_max)
...
def batch_simulator(prior_samples, n_obs):   
    """
    Simulate a batch of oscillation model datasets with n_obs observations each
    """    
    ...
    for m in range(n_sim):
        ...
        sim_data[m, :, 0] = np.sin(a*2*np.pi*time_points[0:n_obs]) + b + np.random.normal(0, sigma, size=n_obs)
...
trainer = ParameterEstimationTrainer(
    ...
    checkpoint_path = './Osc(20-41)_5ACB_[64,64,64]_LSTM(128)_ckpts',
    max_to_keep=300,
    skip_checks=True
)
...
losses = trainer.train_online(epochs=300, iterations_per_epoch=1000, batch_size=128, n_obs=prior_N)
print(losses)
